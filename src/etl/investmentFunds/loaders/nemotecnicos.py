...
from src.db.models.fondos_inversion import NemotecnicoFI
...
def load_nemotecnicos_fi(path: Path) -> int:
    records = _parse_html(path.read_text(encoding="utf-8"))

    with SessionLocal() as session:
        stmt = insert(NemotecnicoFI).values(records)
    ...
    logger.info("Nemotecnicos FI: %d cuotas upserted", len(records))
    return len(records)
